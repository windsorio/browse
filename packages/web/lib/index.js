...
const getNewScope = (parent, page = null) => {
  if (!parent) {
    parent = getSTD({ evalAsyncRuleSet, getNewScope });
  }
  return {
    fns: {},
    vars: {},
...
const evalAsyncRule = async (rule, scope) => {
  assert(rule.type === "Rule");
  const { fn, args } = rule;
  assert(fn.type === "Word");
  ...
  const retVal = await resolveFn(fn.name, scope)(scope)(...resolvedArgs);
  return retVal;
};
...
const evalAsyncRuleSet = async (ruleSet, parent) => {
  assert(ruleSet.type === "RuleSet");
  ...
  for (let i = 0; i < rules.length; i++) {
    const rule = rules[i];
    await evalAsyncRule(rule, scope);
  }
  return await evalAsyncRule(lastRule, scope);
};
