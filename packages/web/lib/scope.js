...
/**
 * Recurseivlely find a meta variable matching the name by walking up the scope/environment inheritance chain
 * @param {string} name The variable name
 * @param {Scope} scope The scope to use
 */
const resolveMeta = (name, scope) => {
  if (!scope) {
    throw new Error(`Internal Error:: Meta Variable '${name}' is not defined`);
  }
  if (scope.meta[name] !== undefined) {
    return scope.meta[name];
  } else {
    return resolveMeta(name, scope.parent);
  }
};
